...
///
/// ### 설계 원칙:
/// - **Actor 기반 동시성**: 모든 내부 상태(`containerCache`, `weakReferences` 등)는 `actor`에 의해 보호되어,
///   여러 스레드에서 동시에 접근해도 데이터 경쟁(Data Race)으로부터 안전합니다.
/// - **단일 책임 원칙 (SRP)**: 의존성 해결, 인스턴스 생성, 캐싱, 약한 참조 관리 등 각 기능은
///   명확히 분리된 메서드(`getOrCreate...`, `create...`, `setup...`)에서 처리됩니다.
/// - **명확한 API**: `.weak` 스코프는 타입 안정성을 위해 `WeaverBuilder`의 `registerWeak` 메서드를 통해서만
///   등록되도록 강제하여 API의 오용 가능성을 원천적으로 차단합니다.
